...
def parse_field(player: int, height, width):
    ...
    while enemy_positions != enemy_positions_done:
        ...
        for xer in range(3):
            for yer in range(3):
                try:
                    if 0 > pos[1] + yer - 1 or pos[1] + yer - 1 > height or 0 > pos[0] + xer - 1 or height < pos[0] + xer - 1:
                        pass
    ...
    while True:
        ...
        while nums_positions != nums_positions_done:
            ...
            for xer,yer in [(1,0), (1,2), (0,1), (2,1)]:
                try:
                    if pos[1] + yer - 1 < 0 or pos[0] + xer - 1 < 0 or pos[1] + yer - 1 > height - 1 or pos[0] + xer - 1 > width - 1:
                        pass
                    elif field[pos[1] + yer - 1][pos[0] + xer - 1] == "_":
...
def checker(player, field, field_height, field_width, xer, yer, figure, height, width):
    ...
    for line in range(height):
        for widther in range(width):
            try:
                if figure[line][widther] == "*" and (zone[line][widther] == (("O" or "o") if player == 1 else ("X" or "x"))):
                    counter += 1
