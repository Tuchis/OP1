...
def parse_field(player: int, height, width):
    ...
    while enemy_positions != enemy_positions_done:
        ...
        for xer in range(3):
            for yer in range(3):
                try:
                    if 0 > pos[1] + yer - 1 or pos[1] + yer - 1 > \
                            height or 0 > pos[0] + xer - 1 or height < pos[0] + xer - 1:
                        pass
                    elif field[pos[1] + yer - 1][pos[0] + xer - 1] == "_":
                        field[pos[1] + yer - 1][pos[0] + xer - 1] = "1"
                    elif field[pos[1] + yer - 1][pos[0] + xer - 1] == \
    ...
    while True:
        ...
        while nums_positions != nums_positions_done:
            ...
            for xer,yer in [(1,0), (1,2), (0,1), (2,1)]:
                try:
                    if pos[1] + yer - 1 < 0 or pos[0] + xer - 1 < 0 \
                            or pos[1] + yer - 1 > height - 1 or pos[0] + xer - 1 > width - 1:
                        pass
                    elif field[pos[1] + yer - 1][pos[0] + xer - 1] == "_":
...
def checker(player, field, field_height, field_width, xer, yer, figure, height, width):
    ...
    """
    price = 0
    if xer + width > field_width or yer + height > field_height:
        return False, price
    zone = []
    for heighter in range(height):
        try:
            zone.append(field[yer + heighter][xer: xer + width])
        except ValueError:
            pass
    counter = 0
        ...
        for widther in range(width):
            try:
                if figure[line][widther] == "*" and (zone[line][widther] \
                            == (("O" or "o") if player == 1 else ("X" or "x"))):
                    counter += 1
                elif figure[line][widther] == "*" and (zone[line][widther]\
                                == (("X" or "x") if player == 1 else ("O" or "o"))):
                    return False, price
    ...
    if counter != 1:
        ...
        for hei in range(field_height):
            for wid in range(field_width):
                if temp_field[hei][wid] == (("O" or "o") \
                        if player == 1 else ("X" or "x")):
...
def mover(player, field, field_height, field_width,\
    ...
    for heighter in range(field_height):
        for elem_index in range(field_width):
            status, price = checker(player, field, field_height,\
                                    field_width, elem_index, heighter, figure, figure_height, figure_width)
            if status == 1:
